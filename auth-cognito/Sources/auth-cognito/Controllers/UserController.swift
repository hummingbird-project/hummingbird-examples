...
final class UserController {
    ...
    func login(_ request: HBRequest) throws -> CognitoAuthenticateResponse {
        guard let authenticateResponse = request.auth.get(CognitoAuthenticateResponse.self) else { throw HBHTTPError(.unauthorized) }
    ...
    func authenticateAccess(_ request: HBRequest) throws -> CognitoAccessToken {
        guard let token = request.auth.get(CognitoAccessToken.self) else { throw HBHTTPError(.unauthorized) }
        return token
    }
    ...
    func attributes(_ request: HBRequest) -> EventLoopFuture<String> {
        ...
        guard let token = request.auth.get(CognitoAccessToken.self) else { return request.failure(.unauthorized) }
    ...
    func authenticateId(_ request: HBRequest) throws -> User {
        guard let token = request.auth.get(User.self) else { throw HBHTTPError(.unauthorized) }
        return token
    }
    ...
    func refresh(_ request: HBRequest) -> EventLoopFuture<CognitoAuthenticateResponse> {
        ...
        guard let refreshToken = request.auth.bearer?.token else { return request.failure(.badRequest) }
    ...
    func mfaGetSecretCode(_ request: HBRequest) -> EventLoopFuture<MfaGetTokenResponse> {
        guard let accessToken = request.auth.bearer else { return request.failure(.unauthorized) }
