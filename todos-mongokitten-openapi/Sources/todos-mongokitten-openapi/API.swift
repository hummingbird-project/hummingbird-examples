...
struct API: APIProtocol {
    ...
    func getTodo(
        ...
        guard let model = try await self.mongo["todos"].findOne("_id" == id, as: TodoModel.self) else {
    ...
    func updateTodo(
        ...
        guard try await self.mongo["todos"].updateEncoded(where: "_id" == id, to: model).updatedCount == 1 else {
